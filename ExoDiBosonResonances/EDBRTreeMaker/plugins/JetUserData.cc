...
void JetUserData::produce( edm::Event& iEvent, const edm::EventSetup& iSetup) {
	...
	for (size_t i = 0; i< jetColl->size(); i++){
		...
		float JERSF_temp        = res_sf.getScaleFactor(jetParam);
                float JEC_up_m_down_1 = smearedP4_JEC_up.pt() - smearedP4_JEC_down.pt();
                float first_get_JER_corr = get_JER_corr(JERSF_temp, isMC, jet, coneSize_, PtResolution, jetCorrFactor);
//		smearedP4_JEC_up *= get_JER_corr(JERSF_temp, isMC, jet, coneSize_, PtResolution, jetCorrFactor);
		smearedP4_JEC_up *= first_get_JER_corr;
                float second_get_JER_corr = get_JER_corr(JERSF_temp, isMC, jet, coneSize_, PtResolution, jetCorrFactor);
//		smearedP4_JEC_down *= get_JER_corr(JERSF_temp, isMC, jet, coneSize_, PtResolution, jetCorrFactor);
		smearedP4_JEC_down *= second_get_JER_corr;
                float JEC_up_m_down = smearedP4_JEC_up.pt() - smearedP4_JEC_down.pt();
...
//qianming 20191110
                ...
                jet.addUserFloat("first_get_JER_corr",   first_get_JER_corr);
                jet.addUserFloat("second_get_JER_corr",   second_get_JER_corr);
                jet.addUserFloat("JEC_up_m_down",   JEC_up_m_down);
                jet.addUserFloat("JEC_up_m_down_1",   JEC_up_m_down_1);
